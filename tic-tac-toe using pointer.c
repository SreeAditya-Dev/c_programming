...
int main() {
    ...
    do {
        ...
        for (int i = 0; i < 9; i++) { // A total of 9 moves
            ...
            // Assign 'X' or 'O' to the array using pointers
...
int check(char (*arr)[3]) {
    // Check row
    for (int i = 0; i < 3; i++) {
        if (*(*(arr + i)) == *(*(arr + i) + 1) && *(*(arr + i) + 1) == *(*(arr + i) + 2) && *(*(arr + i)) != '_') {
            return 1;
        }
    }
    ...
    // Check column
    for (int i = 0; i < 3; i++) {
        if (*(*(arr) + i) == *(*(arr + 1) + i) && *(*(arr + 1) + i) == *(*(arr + 2) + i) && *(*(arr) + i) != '_') {
            return 1;
        }
    }

    // Check diagonal
    if (*(*(arr)) == *(*(arr + 1) + 1) && *(*(arr + 1) + 1) == *(*(arr + 2) + 2) && *(*(arr)) != '_') {
        return 1;
    }
    if (*(*(arr) + 2) == *(*(arr + 1) + 1) && *(*(arr + 1) + 1) == *(*(arr + 2)) && *(*(arr) + 2) != '_') {
        return 1;
    }
